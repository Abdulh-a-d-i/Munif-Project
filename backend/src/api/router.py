...
async def agent_book_google_appointment(request: Request):
    ...
    try:
        ...
        if not is_available:
            logging.warning(f"âš ï¸ Time slot conflict for user {user_id} on {appointment_date} {start_time}-{end_time}")
            return JSONResponse(
                status_code=409,
                content={
                    "success": False,
        ...
        logging.info(f"âœ… Booked appointment for user {user_id}: {title} on {appointment_date} {start_time}-{end_time}")
